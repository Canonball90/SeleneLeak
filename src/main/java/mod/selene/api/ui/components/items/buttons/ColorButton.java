...
public class ColorButton extends Button {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        SeleneLoader.textManager.drawStringWithShadow(getName(), x + 2.3f, y - 1.7f - (float) ClickGUI.getClickGui().getTextOffset(), -1);
        if (setting.isOpen) {
            drawPicker(setting, (int) x + 4, (int) y + 15, (int) x + 4, (int) y + 103, (int) x + 4, (int) y + 95, mouseX, mouseY);
            SeleneLoader.textManager.drawStringWithShadow(isInsideCopy(mouseX, mouseY) ? ChatFormatting.UNDERLINE + "Copy" : "Copy", x + 10f, y + 113, -1);
            SeleneLoader.textManager.drawStringWithShadow(isInsidePaste(mouseX, mouseY) ? ChatFormatting.UNDERLINE + "Paste" : "Paste", x + 45f, y + 113, -1);
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        ...
        if (mouseButton == 0 && isInsidePaste(mouseX, mouseY) && setting.isOpen) {
            try {
                ...
                Command.sendMessage("Not a color format! Available color formats:");
                Command.sendMessage("RGB: (Red), (Green), (Blue)");
                Command.sendMessage("HEX: #FFFFFF - must start with a hashtag '#'");
            }
        }
    }
    ...
    public boolean isInsideCopy(int mouseX, int mouseY) {
        return mouseOver((int) x + 10, (int) y + 113, (int) (x + 10) + SeleneLoader.textManager.getStringWidth("Copy"), (int) (y + 112) + SeleneLoader.textManager.getFontHeight(), mouseX, mouseY);
    }

    public boolean isInsidePaste(int mouseX, int mouseY) {
        return mouseOver((int) x + 45, (int) y + 113, (int) (x + 45) + SeleneLoader.textManager.getStringWidth("Paste"), (int) (y + 112) + SeleneLoader.textManager.getFontHeight(), mouseX, mouseY);
    }
    ...
    void drawHueSlider(int x, int y, int width, int height, float hue) {
        ...
        if (height > width) {
            ...
            RenderUtil.drawOutlineRect(x, sliderMinY - 1, x + width, sliderMinY + 1, 0);
        } else {
            for (int colorIndex = 0; colorIndex < 6; colorIndex++) {
                int previousStep = Color.HSBtoRGB((float) step / 6, 1.0f, 1.0f);
                int nextStep = Color.HSBtoRGB((float) (step + 1) / 6, 1.0f, 1.0f);
            ...
            RenderUtil.drawOutlineRect(sliderMinX - 1, y, sliderMinX + 1, y + height, 0);
        }
    }
