...
import mod.selene.injections.inj.IEntityLivingBase;
...
public class EntityUtil implements Util {
    ...
    public static void attackCrystals(BlockPos pos, boolean rotate) {
        ...
        int ping = SeleneLoader.serverManager.getPing();
        ...
        for (EntityEnderCrystal crystal : mc.world.getEntitiesWithinAABB(EntityEnderCrystal.class, new AxisAlignedBB(pos))) {
            ...
            if (attackTimer.passedMs(ping <= 50 ? 75 : 100)) {
                ...
                if (rotate) {
                    Vec3d crystalVec3d = crystal.getPositionVector();
                    RotationUtil.rotatePacket(crystalVec3d);
                }
    ...
    public static boolean isCrystalAtFeet(EntityEnderCrystal crystal, double range) {
        for (EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            if (EntityUtil.mc.player.getDistanceSq(player) > range * range || SeleneLoader.friendManager.isFriend(player))
                continue;
    ...
    public static boolean isPlayerValid(final EntityPlayer player, final double range) {
        return player != null && player != mc.player && mc.player.getDistanceSq(player) < range * range && !isDead(player) && !SeleneLoader.friendManager.isFriend(player.getName());
    }
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || isDead(entity) || entity.equals(mc.player) || (entity instanceof EntityPlayer && SeleneLoader.friendManager.isFriend(entity.getName())) || mc.player.getDistanceSq(entity) > Math.pow(range, 2);
    }
    ...
    public static Color getColor(Entity entity, int red, int green, int blue, int alpha, boolean colorFriends) {
        ...
        if (entity instanceof EntityPlayer) {
            if (colorFriends && SeleneLoader.friendManager.isFriend((EntityPlayer) entity)) {
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : mc.world.playerEntities) {
            ...
            output.put(healthSB + " " + (SeleneLoader.friendManager.isFriend(player) ? TextUtil.AQUA : TextUtil.RESET) + player.getName() + " " + distanceSB + " " + TextUtil.WHITE + SeleneLoader.totemPopManager.getTotemPopString(player) + SeleneLoader.potionManager.getTextRadarPotion(player), (int) mc.player.getDistance(player));
