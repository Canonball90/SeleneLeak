...
public class NoRender extends Module {
    ...
    public void onUpdate() {
        if (items.getValue()) {
            Util.mc.world.loadedEntityList.stream().filter(EntityItem.class::isInstance).map(EntityItem.class::cast).forEach(Entity::setDead);
        }

        if (noWeather.getValue() && Util.mc.world.isRaining()) {
            Util.mc.world.setRainStrength(0);
        }
    }
    ...
    public void onRenderPost(RenderGameOverlayEvent.Post event) {
        if (event.getType() == RenderGameOverlayEvent.ElementType.BOSSINFO && boss.getValue() != Boss.NONE) {
            if (boss.getValue() == Boss.MINIMIZE) {
                Map<UUID, BossInfoClient> map = Util.mc.ingameGUI.getBossOverlay().mapBossInfos;
                if (map == null) return;
                ...
                for (Map.Entry<UUID, BossInfoClient> entry : map.entrySet()) {
                    ...
                    if (!event.isCanceled()) {
                        ...
                        Util.mc.getTextureManager().bindTexture(GUI_BARS_TEXTURES);
                        Util.mc.ingameGUI.getBossOverlay().render(k, j, info);
                        Util.mc.fontRenderer.drawStringWithShadow(text, (i / scale.getValue()) / 2 - Util.mc.fontRenderer.getStringWidth(text) / 2, (float) (j - 9), 16777215);
                    }
                    GL11.glScaled(1d / scale.getValue(), 1d / scale.getValue(), 1);
                    j += 10 + Util.mc.fontRenderer.FONT_HEIGHT;
                }
