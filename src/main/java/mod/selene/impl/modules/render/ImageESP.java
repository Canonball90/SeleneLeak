...
import static mod.selene.impl.modules.misc.ElytraSwap.drawCompleteImage;


public class ImageESP extends Module {
    private final int black = Color.BLACK.getRGB();
    private final Setting<Color> cc = this.register(new Setting<>("CustomColor", new Color(255, 255, 255, 255)));
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (Mode2.getValue() == mode2.Original) {
            Color original = new Color(255, 255, 255);
            color = original.getRGB();
        }

        float scale = 1;

        for (Entity entity : mc.world.loadedEntityList) {
            if (isValid(entity) && RenderUtil.isInViewFrustum(entity)) {
                ...
                if (entityPlayer != ImageESP.mc.player) {
                    ...
                    if (position != null) {
                        ...
                        if (Mode3.getValue() == mode3.CAT) {
                            Minecraft.getMinecraft().getTextureManager().bindTexture(new ResourceLocation("textures/cat.png"));
                        }
    ...
    public enum mode2 {
        Custom, Rainbow, Astolfo, Original
