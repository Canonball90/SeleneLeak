...
public class Surround extends Module {
    ...
    private final Setting<Timing> timing =
            register(new Setting<>("Timing", Timing.VANILLA));
    private final Setting<Integer> tickDelay =
            register(new Setting<>("TickDelay", 50, 0, 250));
    private final Setting<Integer> blocksPerTick =
            register(new Setting<>("BPT", 2, 1, 30));
    private final Setting<Boolean> rotate =
            register(new Setting<>("Rotate", true));
    private final Setting<Boolean> packet =
            register(new Setting<>("Packet", true));
    private final Setting<Boolean> noLag =
            register(new Setting<>("NoLag", true));
    private final Setting<Boolean> clean =
            register(new Setting<>("Clean", true));
    private final Setting<Boolean> jumpDisable =
            register(new Setting<>("JumpDisable", true));
    private final Setting<Swap> swap =
            register(new Setting<>("Swap", Swap.NORMAL));
    private final Setting<Center> center =
            register(new Setting<>("Center", Center.NONE));

    private final Setting<Boolean> render =
            register(new Setting<>("Render", true));
    private final Setting<Boolean> box =
            register(new Setting<>("Box", true, v -> render.getValue()));
    private final Setting<Boolean> line =
            register(new Setting<>("Line", true, v -> render.getValue()));
    private final Setting<Color> color =
            register(new Setting<>("Color", new Color(255, 255, 255, 100), v -> render.getValue()));
    private final Setting<Boolean> rainbow =
            register(new Setting<>("Rainbow", true));
    private final Setting<Integer> rainbowDelay = register(new Setting<>("RainbowDelay", 100, 0, 255, v -> render.getValue() && rainbow.getValue()));
    ...
    public Surround() {
        super("Surround", "Surrounds your feet with obby.", Category.COMBAT, true, false, false);
    }
        ...
        if (render.getValue()) {
            ...
            renderBlocks.forEach((pos, time) -> {
                ...
                if (System.currentTimeMillis() - time > 400) {
                    ...
                    Color color2 = rainbow.getValue() ? PaletteHelper.rainbow(rainbowDelay.getValue(), 150, 150) : color.getValue();

                    RenderUtil.drawBoxESP(
                            new BlockPos(pos),
                            color2,
                            false,
                            new Color(255, 255, 255, color2.getAlpha()),
                            0.7f,
                            line.getValue(),
                            box.getValue(),
    ...
    private void placeBlock(BlockPos pos, int blockSlot, int oldSlot) {
        ...
        BlockUtil.placeBlock(pos, rotate.getValue(), packet.getValue(), clean.getValue(), false);
