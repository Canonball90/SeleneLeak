...
import mod.selene.world.TotemPopEvent;
...
public class PopLagger extends Module {
    ...
    public final Setting<Mode> messageMode = this.register(new Setting<Mode>("Mode", Mode.DM));
    private final Setting<Float> delay = this.register(new Setting("Delay(Secs)", 5, 1, 10));
    private final Setting<Boolean> message = this.register(new Setting("Message", true));
    private final Setting<Boolean> cancelMessage = this.register(new Setting("CancelMessage", false));
    private final Timer timer = new Timer();

    public PopLagger() {
        super("PopLag", "Does trolling things.", Category.OTHER, true, false, false);
    }
    ...
    public void onPop(TotemPopEvent event) {
        ...
        if (!timer.passed((long) (delay.getValue() * 1000))) return;
        {
            ...
            if (message.getValue())
                Command.sendMessage("Sent lag text to " + event.getEntity().getDisplayName().getUnformattedText() + ".");
            timer.reset();
            {
                mc.player.sendChatMessage(lagText);
                if (message.getValue()) Command.sendMessage("Sent lag text to Global chat.");
                timer.reset();
            }
        }
    }
